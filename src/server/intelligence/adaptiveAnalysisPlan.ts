...
const PIPELINE_INPUT_CAPS: Record<PipelineType, number> = {
  short_direct: AI_CONFIG.input.maxChars,
  medium_compacted: 14_000,
  long_preview: 9_000,
};
...
export function createAdaptiveAnalysisPlan(
  ...
  if (tokenBudget.riskLevel === "high" && pipelineType === "short_direct") {
    pipelineType = "medium_compacted";
  }

  const maxInputCharacters = Math.min(
    PIPELINE_INPUT_CAPS[pipelineType],
    AI_CONFIG.input.maxChars,
  );
